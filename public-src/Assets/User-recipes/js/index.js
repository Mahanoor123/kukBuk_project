...
function submitbtn() {
  ...
  const servingInput = document.querySelector(
    'input[placeholder="4 persons "]'
  );
  ...
  const formData = {
    file:
      JSON.parse(localStorage.getItem("formData"))?.file || "No file selected", // Retrieve from localStorage
  ...
  if (
    !userCollection.some(
      (existingRecipe) =>
        existingRecipe.recipeTitle === formData.recipeTitle &&
        existingRecipe.description === formData.description
    )
  ) {
...
function closePopup() {
  ...
  window.location.href = "../html/userRecipe.html";
}
