...
document?.addEventListener("DOMContentLoaded", function () {
  ...
  form?.addEventListener("submit", async function (e) {
    ...
    const recipeCategory = document.querySelectorAll(
      'input[name="form-check-input"]:checked'
    );
    const recipeCategoryValues = Array.from(recipeCategory).map(
      (cb) => cb.value
    );
    const recipeIngredients = Array.from(
      ingredientList.querySelectorAll("li")
    ).map((li) => li.textContent);
    const recipeCookingSteps = Array.from(stepList.querySelectorAll("li")).map(
      (li) => li.textContent
    );
    const recipeDescription =
      document.getElementById("floatingTextarea3").value;
